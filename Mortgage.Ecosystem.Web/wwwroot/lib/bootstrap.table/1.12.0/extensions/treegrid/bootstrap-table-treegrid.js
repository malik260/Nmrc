    ...
    $.extend($.fn.bootstrapTable.defaults, {
        ...
        onGetNodes: function (row, data) {
            ...
            $.each(data, function (i, item) {
                if (row[that.options.idField] === item[that.options.parentField]) {
                    nodes.push(item);
                }
            });
            return nodes;
        ...
        onCheckRoot: function (row, data) {
            var that = this;
            return !row[that.options.parentField];
        }
    });
    ...
    var initTr = function (item, idx, data, parentDom) {
        ...
        for (var i = 0; i <= len; i++) {
            ...
            that.options.rowStyle = function (item, idx) {
                ...
                var pid = item[that.options.parentField] ? item[that.options.parentField] : 0;
                return {
