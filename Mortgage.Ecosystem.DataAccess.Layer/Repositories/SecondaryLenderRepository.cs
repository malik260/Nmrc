...
{
    public class SecondaryLenderRepository : DataRepository, ISecondaryLenderRepository
    {
        #region Retrieve data
        public async Task<List<SecondaryLenderEntity>> GetList(SecondaryLenderListParam param)
        {
        ...
        public async Task<List<SecondaryLenderEntity>> GetPageList(SecondaryLenderListParam param, Pagination pagination)
        {
            var expression = ListFilter(param);
        ...
        public async Task<List<SecondaryLenderEntity>> GetApprovalPageList(SecondaryLenderListParam param, Pagination pagination)
        {
            ...
            // Get approval items
            var approvalItems = await new DataRepository().GetSecondaryLendersApprovalItems();
        ...
        public async Task<SecondaryLenderEntity> GetEntity(long id)
        {
            return await BaseRepository().FindEntity<SecondaryLenderEntity>(x => x.Id == id);
        }
        public async Task<SecondaryLenderEntity> GetEntitybyNhf(string nhf)
        {
            return await BaseRepository().FindEntity<SecondaryLenderEntity>(x => x.NHFNumber == nhf || x.Name == nhf);
        }

        public async Task<SecondaryLenderEntity> GetEntitybyEmail(string email)
        {
            return await BaseRepository().FindEntity<SecondaryLenderEntity>(x => x.EmailAddress == email);
        }

        public async Task<SecondaryLenderEntity> GetEntitybyName(string SecondaryLenderName)
        {
        ...
        public bool ExistRCNumber(SecondaryLenderEntity entity)
        {
            var expression = ExtensionLinq.True<SecondaryLenderEntity>();
            expression = expression.And(t => t.BaseIsDelete == 0);
            if (entity.Id.IsNullOrZero())
        ...
        #region Submit data
        public async Task SaveForm(SecondaryLenderEntity entity)
        {
            if (entity.Id.IsNullOrZero() || entity.Id.ToStr().Length < 15)
            {
                await entity.Create();
                await BaseRepository().Insert<SecondaryLenderEntity>(entity);
            }
            else
            {
                await entity.Modify();
                await BaseRepository().Update<SecondaryLenderEntity>(entity);
            }
        }

        public async Task SaveForms(SecondaryLenderEntity entity)
        {
            var message = string.Empty;
            var db = await BaseRepository().BeginTrans();
            try
        ...
        public async Task DeleteForm(string ids)
        {
            ...
            await BaseRepository().Delete<SecondaryLenderEntity>(idArr);
        }

        public async Task<bool> ApproveForm(SecondaryLenderEntity entity, MenuEntity menu, OperatorInfo user)
        {
            var message = string.Empty;
        ...
        public async Task<bool> DisApproveForm(SecondaryLenderEntity entity)
        {
            var message = string.Empty;
            var approvalLog = new ApprovalLogEntity();
            var db = await BaseRepository().BeginTrans();
        ...
        private Expression<Func<SecondaryLenderEntity, bool>> ListFilter(SecondaryLenderListParam param)
        {
            var expression = ExtensionLinq.True<SecondaryLenderEntity>();
            if (param != null)
            {
